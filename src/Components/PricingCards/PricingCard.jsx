const PricingCard = ({ pricing }) => {
  return (
    <div className="border border-b-cyan-100 rounded-md p-3">
      <div className="card w-96 bg-base-100 shadow-sm h-full">
        <div className="card-body flex flex-col h-full">
          {pricing.isPopular && <span className="badge badge-xs badge-warning">Most Popular</span>}
          <div className="flex justify-between">
            <h2 className="text-3xl font-bold">{pricing.name}</h2>
            <span className="text-xl">${pricing.price}/{pricing.duration}</span>
          </div>
